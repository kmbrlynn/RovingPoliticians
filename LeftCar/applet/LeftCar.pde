...
void loop(){
  if (digitalRead(switchPin) == HIGH) {    //if the switch is turned on
    ...
    digitalWrite(leftMotor1, LOW);                 // left motor stop
...
void carForwards(){                        // function that makes the car go forwards
  ...
  digitalWrite(rightMotor1, LOW);                  // right motor goes forwards
...
void carReverse(){                         // function that makes the car reverse
  ...
  digitalWrite(leftMotor2, LOW);
  digitalWrite(rightMotor1, HIGH);                 // right motor goes backwards
  digitalWrite(rightMotor2, LOW);
  delay(750);
}

void carPivot(){                           //function that makes the car pivot
  Serial.println("pivot left!");                   // serial monitor for car turning left
  digitalWrite(leftMotor1, HIGH);                  // left motor goes forwards
  digitalWrite(leftMotor2, LOW);
...
void lightUp(){                            //function that turns the LED on for 5 seconds
